...
[source,sh]
----
oc --config /home/kni/dev-scripts/ocp/auth/kubeconfig get pod --all-namespaces -o=custom-columns=NAME:.metadata.name | grep operator | sort
----
+
.Sample Output
[source,texinfo]
...
----
CLUSTERID=$(oc --config /home/kni/dev-scripts/ocp/auth/kubeconfig get machineset -n openshift-machine-api -o jsonpath='{.items[0].metadata.labels.machine\.openshift\.io/cluster-api-cluster}')
echo $CLUSTERID
----
